...
function addClickHandler(document, book) {
  ...
  document.querySelectorAll('img').forEach(img => {
    let pressTimer = null; // Variável para armazenar a referência do timer

    // Função que inicia o processo de "long press"
    const startPress = (e) => {
      // Previne comportamentos padrão do navegador, como arrastar a imagem.
      e.preventDefault();

      // Define um cronômetro para executar a ação após 1000ms (1 segundo)
      pressTimer = window.setTimeout(async () => {
        console.log("Long press detectado!");
        try {
          const imagePath = getKeyByValue(book.archive.urlCache, img.src);

          if (!imagePath) {
            console.error("Caminho da imagem não encontrado no cache para a src:", img.src);
            return;
          }

          const base64 = await book.archive.getBase64(imagePath);

          if (base64) {
            // Chama o handler do Flutter para enviar o base64
            window.flutter_inappwebview.callHandler('imageClickHandler', base64);
          } else {
            console.error("Não foi possível carregar o conteúdo do arquivo:", imagePath);
          }
        } catch (error) {
          console.error("Ocorreu um erro ao processar a imagem:", error);
        }
      }, 800); // 1000 ms = 1 segundo
    };

    // Função que cancela o "long press" se o usuário soltar antes do tempo
    const cancelPress = () => {
      // Limpa o cronômetro para que a ação não seja executada
      clearTimeout(pressTimer);
    };

    // Adiciona os ouvintes de evento para mouse (desktop)
    img.addEventListener('mousedown', startPress);
    img.addEventListener('mouseup', cancelPress);
    img.addEventListener('mouseleave', cancelPress); // Cancela se o mouse sair da imagem

    // Adiciona os ouvintes de evento para toque (mobile)
    img.addEventListener('touchstart', startPress);
    img.addEventListener('touchend', cancelPress);
    img.addEventListener('touchcancel', cancelPress); // Cancela se o toque for interrompido
  });
}
